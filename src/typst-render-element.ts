export default class TypstRenderElement extends HTMLElement {
  ...
  async connectedCallback() {
    if (!this.isConnected) {
      console.warn(
        "Typst Renderer: Canvas element has been called before connection"
      );
      return;
    }

    if (this.format == "image" && this.canvas == undefined) {
      this.canvas = this.appendChild(
        createEl("canvas", {
          attr: { height: TypstRenderElement.prevHeight },
          cls: "typst-doc",
        })
      );
    }
    ...
    try {
      await navigator.locks.request(
        ...
        async () => {
          ...
          try {
            const result = await TypstRenderElement.compile(
              this.path,
              this.source,
              this.size,
              this.display,
            ...
            if (result instanceof ImageData && this.format == "image") {
              ...
              const child = this.firstElementChild as SVGElement;
              child.setAttribute(
                "width",
                child.getAttribute("width")!.replace("pt", "")
              );
              child.setAttribute(
                "height",
                child.getAttribute("height")!.replace("pt", "")
              );
              child.setAttribute(
                "width",
                `${this.firstElementChild!.clientWidth / fontSize}em`
              );
              child.setAttribute(
                "height",
                `${this.firstElementChild!.clientHeight / fontSize}em`
              );
            }
          } catch (error) {
