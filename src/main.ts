...
export default class TypstPlugin extends Plugin {
  ...
  async onload() {
    ...
    if (
      !(await this.app.vault.adapter.exists(this.wasmPath)) ||
      ...
      const fonts = await Promise.all(
        ...
        ((await window.queryLocalFonts()) as Array)
          .filter((font: { family: string; name: string }) =>
            this.settings.font_families.contains(font.family.toLowerCase())
          )
          .map(
    ...
    // Setup cutom canvas
    TypstRenderElement.compile = (a, b, c, d, e) =>
      this.processThenCompileTypst(a, b, c, d, e);
    ...
    this.registerMarkdownCodeBlockProcessor(
      ...
      async (source, el, ctx) => {
        el.appendChild(
          this.createTypstRenderElement(
            "/" + ctx.sourcePath,
            `${this.settings.preamable.code}\n${source}`,
            true,
            false
          )
        );
  ...
  async fetchWasm() {
    ...
    let data;
    response = requestUrl(
      `https://api.github.com/repos/fenjalien/obsidian-typst/releases/tags/${PLUGIN_VERSION}`
    );
    data = await response.json;
    const asset = data.assets.find(
      (a: any) => a.name == "obsidian_typst_bg.wasm"
    );
    if (asset == undefined) {
  ...
  async getPackageList(): Promise<string[]> {
    const getFolders = async (f: string) =>
      (await this.app.vault.adapter.list(f)).folders;
    const packages = [];
    // namespace
        ...
        while (true) {
          ...
          try {
            ...
            if (
              ...
              e.startsWith("Uncaught Error: package not found (searched for")
            ) {
              const spec = e
                .match(/"@preview\/.*?"/)[0]
                .slice(2, -1)
  ...
  async handleWorkerRequest({ buffer: wbuffer, path }: WorkerRequest) {
    try {
      ...
      if (text) {
        const buffer = Int32Array.from(this.textEncoder.encode(text));
  ...
  async fetchPackage(folder: string, name: string, version: string) {
    ...
    await untar(
      decompressSync(new Uint8Array(await response.arrayBuffer())).buffer
    ).progress(async (file: any) => {
      // is folder
  ...
  async processThenCompileTypst(
    ...
    const sizing = `#let (WIDTH, HEIGHT, SIZE, THEME) = (${
      display ? pxToPt(size) : "auto"
    }, ${!display ? pxToPt(size) : "auto"}, ${pxToPt(
      fontSize
    )}, "${document.body.getCssPropertyValue("color-scheme")}")`;
  ...
  createTypstMath(source: string, r: { display: boolean }) {
    ...
    return this.createTypstRenderElement(
      "/586f8912-f3a8-4455-8a4a-3729469c2cc1.typ",
      source,
      display,
      true
    );
...
class TypstSettingTab extends PluginSettingTab {
  ...
  async display() {
    ...
    const no_fill = new Setting(containerEl)
      ...
      .addToggle((toggle) => {
        toggle.setValue(this.plugin.settings.noFill).onChange(async (value) => {
    ...
    const fill_color = new Setting(containerEl)
      .setName("Fill Color")
      .setDisabled(
        this.plugin.settings.noFill || this.plugin.settings.format == "svg"
      )
      .addColorPicker((picker) => {
        picker.setValue(this.plugin.settings.fill).onChange(async (value) => {
    ...
    const pixel_per_pt = new Setting(containerEl)
      ...
      .addSlider((slider) =>
        slider
          .setValue(this.plugin.settings.pixel_per_pt)
          .setLimits(1, 5, 1)
          .onChange(async (value) => {
    ...
    new Setting(containerEl).setName("Shared Preamble").addTextArea((c) =>
      c
        .setValue(this.plugin.settings.preamable.shared)
        .onChange(async (value) => {
          this.plugin.settings.preamable.shared = value;
          await this.plugin.saveSettings();
        })
    );
    new Setting(containerEl).setName("Code Block Preamble").addTextArea((c) =>
      c
        .setValue(this.plugin.settings.preamable.code)
        .onChange(async (value) => {
          this.plugin.settings.preamable.code = value;
          await this.plugin.saveSettings();
        })
    );
    new Setting(containerEl).setName("Math Block Preamble").addTextArea((c) =>
      c
        .setValue(this.plugin.settings.preamable.math)
        .onChange(async (value) => {
    ...
    if (!Platform.isMobileApp) {
      const fontSettings = containerEl.createDiv({
        cls: "setting-item font-settings",
      });
      fontSettings.createDiv({ text: "Fonts", cls: "setting-item-name" });
      fontSettings.createDiv({
        text: "Font family names that should be loaded for Typst from your system. Requires a reload on change.",
        cls: "setting-item-description",
      });

      const addFontsDiv = fontSettings.createDiv({
        cls: "add-fonts-div",
      });
      const fontsInput = addFontsDiv.createEl("input", {
        type: "text",
      ...
      const fontTagsDiv = fontSettings.createDiv({
        cls: "font-tags-div",
      });

      const addFontTag = async () => {
        if (!this.plugin.settings.font_families.contains(fontsInput.value)) {
          this.plugin.settings.font_families.push(
            fontsInput.value.toLowerCase()
          );
          await this.plugin.saveSettings();
        }
        fontsInput.value = "";
      ...
      new Setting(containerEl)
        ...
        .setDesc(
          "When on, if the compiler cannot find a package in the system it will attempt to download it. Packages downloaded this way will be stored within the vault in the plugin's folder. Always on for mobile."
        )
        .addToggle((toggle) =>
          toggle
    ...
    const packageSettingsDiv = containerEl.createDiv({
      cls: "setting-item package-settings",
    });
    packageSettingsDiv.createDiv({
      text: "Downloaded Packages",
      cls: "setting-item-name",
    });
    packageSettingsDiv.createDiv({
      text: "These are the currently downloaded packages. Select the packages you want to delete.",
      cls: "setting-item-description",
    });
    ...
    (await this.plugin.getPackageList()).forEach((pkg) => {
      ...
      //create package item
      const packageItem = packageSettingsDiv.createDiv({
        cls: "package-item",
      });
      packageItem.createEl("input", {
        type: "checkbox",
        cls: "package-checkbox",
        value: pkg,
        attr: { name: "package-checkbox" },
      });
      packageItem.createEl("p", { text: name });
      packageItem.createEl("p", {
        text: version,
        cls: "package-version",
      });
    });

    const deletePackagesBtn = packageSettingsDiv.createEl("button", {
      text: "Delete Selected Packages",
      cls: "delete-pkg-btn",
    });

    deletePackagesBtn.addEventListener("click", () => {
      const selectedPackageElements = packageSettingsDiv.querySelectorAll(
        'input[name="package-checkbox"]:checked'
      );
  ...
  renderFontTags(fontTagsDiv: HTMLDivElement) {
    ...
    this.plugin.settings.font_families.forEach((fontFamily) => {
      ...
      fontTag.createEl("span", {
        text: fontFamily,
        cls: "font-tag-text",
        attr: { style: `font-family: ${fontFamily};` },
      });
      const removeBtn = fontTag.createEl("span", {
        text: "x",
        cls: "tag-btn",
      });
