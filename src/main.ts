	...
	async onload() {
		...
		if (
			...
			try {
				...
				new Notice(
					"Typst Renderer: Web assembly component downloaded!",
					5000
				);
			} catch (error) {
				new Notice(
					"Typst Renderer: Failed to fetch component: " + error,
					0
				);
				console.error(
					"Typst Renderer: Failed to fetch component: " + error
			...
			const fonts = await Promise.all(
				...
				((await window.queryLocalFonts()) as Array)
					.filter((font: { family: string; name: string }) =>
						this.settings.font_families.contains(
				...
				while (true) {
					...
					try {
						result = await new Promise((resolve, reject) => {
							...
							const remove = () => {
								this.compilerWorker.removeEventListener(
									"message",
									listener
								);
								this.compilerWorker.removeEventListener(
							...
							this.compilerWorker.addEventListener(
								"message",
								listener
							);
							this.compilerWorker.addEventListener(
						...
						if (
							...
							e.startsWith(
								"Uncaught Error: package not found (searched for"
							)
						) {
							const spec = e
								.match(/"@preview\/.*?"/)[0]
...
class TypstSettingTab extends PluginSettingTab {
	...
	async display() {
		...
		const no_fill = new Setting(containerEl)
			...
			.addToggle((toggle) => {
				toggle
					.setValue(this.plugin.settings.noFill)
					.onChange(async (value) => {
		...
		const fill_color = new Setting(containerEl)
			.setName("Fill Color")
			.setDisabled(
				this.plugin.settings.noFill ||
					this.plugin.settings.format == "svg"
			)
		...
		if (!Platform.isMobileApp) {
			...
			const addFontTag = async () => {
				if (
					!this.plugin.settings.font_families.contains(
						fontsInput.value
					)
				) {
